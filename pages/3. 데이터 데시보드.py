...
# 페이지 설정
st.set_page_config(page_title="검증 데이터 데시보드", page_icon="📊", layout="wide")

st.title("📊 모델 검증 및 탐지 통계 데시보드")
...
if not history_data:
    st.info("💡 아직 저장된 탐지 이력이 없습니다. 'MRI 탐지 & 분석' 페이지에서 분석을 수행해 보세요.")
else:
    ...
    avg_conf = df[df['max_confidence'] > 0]['max_confidence'].mean() if not df[df['max_confidence'] > 0].empty else 0
    
    m1.metric("총 분석 횟수", f"{total_count}건")
    m2.metric("종양 검출률", f"{(detected_count/total_count if total_count > 0 else 0):.1%}")
    m3.metric("평균 신뢰도", f"{avg_conf:.1%}")
    m4.metric("최근 분석", df['created_at'].iloc[0].strftime('%m-%d %H:%M') if not df.empty else "-")
    ...
    with col_chart1:
        ...
        if not df[df['max_confidence'] > 0].empty:
            st.bar_chart(df[df['max_confidence'] > 0]['max_confidence'])
    ...
    with comp_col2:
        ...
        if os.path.exists(gt_path):
            st.image(gt_path, caption="[YOLOv8 추론 결과]", use_container_width=True)
